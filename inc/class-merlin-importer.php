...
class Merlin_Importer {
    ...
    function import( $file ) {
        ...
        if ( ! empty( $content['users'] ) ) {
            $this->phoen_users_import( $content['users'] );
        }
     ...
     */
    function phoen_users_import( array $users ) {
    ...
    function importRevSliders( $file ) {
        ...
        $merlin_obj = new RevSlider();
        $response = $merlin_obj->importSliderFromPost(true, true, $file);
     ...
     */
    function phoen_import_theme_data( $customize_data = array() ) {
        global $wp_customize;
